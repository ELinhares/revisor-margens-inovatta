...
def _clean_margin_series(series: pd.Series) -> pd.Series:
    """Remove %, replace comma decimal separator, and strip whitespace."""
...
def _detect_margin_format(series: pd.Series) -> str:
    ...
    """
    valid = pd.to_numeric(_clean_margin_series(series), errors="coerce").dropna()
    valid = valid[valid != 0]
    if len(valid) == 0:
        return "percent"
...
def check_columns(file_bytes: bytes) -> dict:
    ...
    if "Margem Atual" in df_preview.columns:
        df_preview["Margem Atual"] = pd.to_numeric(
            _clean_margin_series(df_preview["Margem Atual"]), errors="coerce"
        )
...
def validate_and_read(
    ...
    df["Margem Atual"] = pd.to_numeric(_clean_margin_series(df["Margem Atual"]), errors="coerce").fillna(0.0)
